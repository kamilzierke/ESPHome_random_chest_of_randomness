...
enum class ClientControlCmd : uint8_t { Unknown = 0, PauseStream = 1, RequestKeyframe = 2 };
...
// [type:1][ver:1][cmd:1][value:1] => 4 bytes
struct RWV_PACKED ClientControlPacket {
  MsgType type;
  uint8_t ver;
  ClientControlCmd cmd;
  uint8_t value;
};
...
inline size_t build_client_control_packet(ClientControlCmd cmd, uint8_t value, uint8_t *out) {
  if (!out) return 0;
  ClientControlPacket pkt{};
  pkt.type = MsgType::ClientControl;
  pkt.ver = kProtocolVersion;
  pkt.cmd = cmd;
  pkt.value = value;
  memcpy(out, &pkt, sizeof(pkt));
