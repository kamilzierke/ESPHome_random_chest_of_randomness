...
export class DeviceBroadcaster {
  ...
  public sendFrameChunked(id: string, data: FrameOut, frameId: number, maxBytes = 12_000): void {
    ...
    const frame: OutFrame = { frameId, packets };

    if (st.sending) {
      // latest-frame-wins for queued backlog while another frame is being sent
      if (st.queue.length === 0) {
        st.queue.push(frame);
      } else {
        st.queue[st.queue.length - 1] = frame;
        st.queue.splice(0, st.queue.length - 1);
      }
    } else {
      st.queue.push(frame);
    }
