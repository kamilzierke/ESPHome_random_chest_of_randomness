...
export interface ClientControlPacket {
  cmd: ClientControlCmd;
  value: number;
}
...
export function parseClientControlPacket(buf: Buffer): ClientControlPacket | null {
  if (!Buffer.isBuffer(buf) || buf.length < CLIENT_CONTROL_BYTES) return null;
  if (buf.readUInt8(0) !== MsgType.ClientControl) return null;
  if (buf.readUInt8(1) !== PROTOCOL_VERSION) return null;
